...
```{r include=FALSE}
df <- data.frame(group=c('A','A','B','B','B'),value=rnorm(5))
...
Na <- sum((df$group=='A'))
Nb <- N-Na
ma <- mean(df$value[df$group=='A'])
mb <- mean(df$value[df$group=='B'])

aa <- ((N-2)*Na*Nb*(ma-mb)^2)/(N*(sum((df$value[df$group=='A']-ma)^2)+sum((df$value[df$group=='B']-mb)^2)))

anova(m1)$F


(N-2)*Na*Nb*(ma-mb)^2/(N*sum((df$value-mean(df$value))^2)-Na*Nb*(ma-mb)^2)
aa
